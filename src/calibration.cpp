...
// Globale Variable – **nur hier** wirklich definiert!
...
void performJoystickCalibration() {
    ...
    for (int axis = 0; axis < NUM_JOYSTICK_AXES; axis++) {
        ...
        for (int i = 0; i < samples; i++) {
            // ACHTUNG: Hier anpassen auf die tatsächlichen Analogpins pro Achse!
            int val = analogRead(axis); // z.B. analogRead(joystickPin[axis]);
